...
void SceneShadeWire::initScene()
{
    ...
    ogre = new VBOMesh("../ConsoleApplication1/Mesh/bs_ears.obj");
...
{
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/shadewire.vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader %s failed to compile!\n%s",
               "shadewire.vs", prog.Log().c_str());
        exit(1);
    }
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/shadewire.gs",GLSLShader::GEOMETRY) )
    {
        printf("Geometry shader failed to compile!\n%s",
               prog.Log().c_str());
        exit(1);
    }
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/shadewire.fs",GLSLShader::FRAGMENT))
    {
