...
void SceneSilhouette::render()
{
    ...
    vec3 cameraPos(3.0f * cos(angle),0.0f,1.5f * sin(angle));
...
{
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/silhouette.vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               prog.Log().c_str());
        exit(1);
    }
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/silhouette.gs",GLSLShader::GEOMETRY) )
    {
        printf("Geometry shader failed to compile!\n%s",
               prog.Log().c_str());
        exit(1);
    }
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/silhouette.fs",GLSLShader::FRAGMENT))
    {
