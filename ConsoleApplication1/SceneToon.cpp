...
void SceneToon::render()
{
    ...
    prog.setUniform("Ka", 0.4f, 0.4f, 0.4f);
...
void SceneToon::resize(int w, int h)
{
    ...
    width = w;
    height = h;
    projection = glm::perspective(40.0f, (float)w/h, 0.3f, 100.0f);
}

void SceneToon::compileAndLinkShader()
{
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/toon.vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               prog.Log().c_str());
        exit(1);
    }
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/toon.fs",GLSLShader::FRAGMENT))
    {
