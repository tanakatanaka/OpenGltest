...
void SceneNightVision::pass1()
{
    ...
    projection = glm::perspective(60.0f, (float)width/height, 0.3f, 1000.0f);
...
void SceneNightVision::resize(int w, int h)
{
    ...
    width = w;
    height = h;
    projection = glm::perspective(60.0f, (float)w/h, 0.3f, 1000.0f);
}

void SceneNightVision::compileAndLinkShader()
{
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/nightvision.vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               prog.Log().c_str());
        exit(1);
    }
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/nightvision.fs",GLSLShader::FRAGMENT))
    {
