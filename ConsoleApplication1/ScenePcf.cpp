...
void ScenePcf::initScene()
{
   ...
   // angle = PI/4.0f;
    ...
    plane.reset(new VBOPlane(20.0f, 20.0f, 2, 2));
    float scale = 2.0f;
    torus.reset(new VBOTorus(0.7f * scale,0.3f * scale,50,50));
    mesh.reset(new VBOMesh("../ConsoleApplication1/mesh/building.obj"));
...
//    lightPV = shadowScale * lightProjection * lightView;
    ...
    lightFrustum->setPerspective(100.0f, 1.0f, 0.1f, 1000.0f);
...
{
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/pcf.vs",GLSLShader::VERTEX) )
    {
        printf("Vertex shader failed to compile!\n%s",
               prog.Log().c_str());
        exit(1);
    }
    if( ! prog.CompileShaderFromFile("../ConsoleApplication1/Shader/pcf.fs",GLSLShader::FRAGMENT))
    {
